  ...
  if [ $# -ge 6 ]; then
    ...
    if hash timeout 2>/dev/null; then
        ...
        while [ "$ecode" -eq 124 ] && [ "$try_count" -lt  10 ]; do
            nic=$(timeout $CHROOT_TIME_OUT chroot ${MOUNT_POINT}/chroot-root/gadgetron /usr/local/share/gadgetron/chroot/gadgetron-dependency-query.sh $GT_HOST $GT_PORT $QUERY_OUT $TIME_OUT)
            ecode=$?
            try_count=`expr $try_count + 1`
        done
    else
        chroot ${MOUNT_POINT}/chroot-root/gadgetron /usr/local/share/gadgetron/chroot/gadgetron-dependency-query.sh $GT_HOST $GT_PORT $QUERY_OUT $TIME_OUT
    fi
